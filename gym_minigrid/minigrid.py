...
class Goal(WorldObj):
    def __init__(self, color='green', toggletimes=1, triage_color='darkbrown'):
        self.toggletimes = toggletimes
        self.required_toggle_times = toggletimes
...
class Grid:
    ...
    def is_on_box(self, on_box):
        for i in range(self.width):
            for j in range(self.height):
                obj = self.grid[j * self.width + i]
                if obj is not None:
                    if obj.type == 'box' or obj == 'goal':
                        obj.on_box = on_box
    ...
    def render_tile(
        ...
        key = (agent_dir, highlight, tile_size, previous_record)
        ...
        if agent_dir is not None:
            rect_fn = point_in_rect(0.2, 0.9, 0.2, 0.9)
            if previous_record:
                fill_coords(img, point_in_rect(0.3, 0.8, 0.3, 0.8), (180, 180, 180))  # (200, 200, 200)) # ORANGE
            else:
                fill_coords(img, rect_fn, (0, 255, 255))  # (255, 0, 0)) # RED
        ...
        for j in range(0, self.height):
            for i in range(0, self.width):
                ...
                if agent_pos[0] == i and agent_pos[1] == j:
                    a_dir = agent_dir
                elif prev_pos_mask is not None and prev_pos_mask[i, j] != -1:
                    a_dir = prev_pos_mask[i, j]
                else:
                    a_dir = None
                tile_img = Grid.render_tile(
                    ...
                    tile_size=tile_size,
                    previous_record=True if not agent_here and prev_pos_mask is not None and prev_pos_mask[
                        i, j] != -1 else False
                )
...
class MiniGridEnv(gym.Env):
    ...
    def render(self, mode='human', close=False, highlight=True, tile_size=TILE_PIXELS, hide_invisible=False):
        ...
        self.yaw = 90 * ((self.agent_dir - 1) % 4)
        # Render the whole grid
