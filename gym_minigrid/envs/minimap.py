...
def fix_jump_locations(same_level, above_level, minigrid_index_mapping, jump_index=11):
    ...
    minigrid_index_mapping['color_mapping'][
        jump_index] = 'white'  # 'grey0' # minigrid_index_mapping['color_mapping'][wall_idx]
...
class Minimap(NumpyMap):
    ...
    def __init__(self,
        ...
        self.yellow_victim_lifetime = yellow_victim_lifetime
...
class MinimapForFalcon(Minimap):
    ...
    def __init__(self,
                 ...
                 agent_pos=(6, 6), agent_dir=0,
                 yellow_victim_lifetime=5 * 60, total_game_duration=10 * 60,
                 seconds_per_step=0.5, random_victim_colors=False, difficulty='easy'):
        raw_map = np.load(raw_map_path)
        ...
        # minimap_array = np.rot90(minimap_array, 1)
        # updated_map_path = Path(RESOURCES_DIR, 'falcon_map.npy')
        if difficulty == 'easy':
            updated_map_path = Path(RESOURCES_DIR, 'Falcon_easy.npy')
        elif difficulty == 'medium':
            updated_map_path = Path(RESOURCES_DIR, 'Falcon_medium.npy')
        elif difficulty == 'difficult':
            updated_map_path = Path(RESOURCES_DIR, 'Falcon_difficult.npy')
        else:
            updated_map_path = Path(RESOURCES_DIR, 'raw_map_state.npy')

        self.difficulty = difficulty
        minimap_array = np.load(updated_map_path)
        ...
        self.yellow_victim_lifetime = yellow_victim_lifetime
        self.random_victim_colors = random_victim_colors
        self.initial_map_state = np.zeros((minimap_array.shape[0] + 2, minimap_array.shape[1] + 2), dtype=np.int32)
        self.gt_map_state = np.zeros((minimap_array.shape[0] + 2, minimap_array.shape[1] + 2), dtype=np.int32)
        self.gt_map_state[1:-1, 1:-1] = minimap_array
        self.initial_gt_map_state = self.gt_map_state
        self.initial_map_state[1:-1, 1:-1] = np.load(Path(RESOURCES_DIR, 'raw_map_state.npy'))
        self.initial_map_state[self.initial_map_state == 1] = 0
        self.observed_map_state = self.initial_map_state
        # self.observed_map_state = copy.deepcopy(self.initial_map_state)
...
def encode_map(minimap_array):
    [rows, cols] = minimap_array.shape
    for i in range(rows):
        for j in range(cols):
            if minimap_array[i, j] == 9 or minimap_array[i, j] == 11:
                minimap_array[i, j] = 255
